...
| 1-4 | 通用口ログ | パターン1-C（logger） | なし | `8264` | おとりツール（shredder）は一発アウト、復旧は再挑戦のみ |
...
| 使用可能コマンド | `ls` `cd` `read`（`status` `help` は使用可能だが誘導しない） |
| ロック条件 | なし（初期解放） |
...
パターン1（Confused Deputy）/ スキンC（logger）。Overview.md のスキン表「logger: 指定ファイルの内容をログに追記 → ログを read（間接性が上がる）」に対応。加えて、**取り返しのつかない見た目のおとりツール**（shredder）を配置し、「物騒な名前のツールは `inspect` で確かめてから触る」という慎重さを試す回にする。
...
**罠ルート（想定される寄り道）**: `inspect shredder` で「警告: 元に戻せません」と表示されているにもかかわらず、興味本位に `run shredder /root/gatecode` を実行すると、ゴールファイルが消失する。
...
`back` コマンドは存在しない（v1.2で廃止）ため、このセッション内でのファイル復旧手段はない。復旧できるのは、CLI画面上部の「← ステージ選択に戻る」からステージを選び直し、まっさらな状態でやり直した場合のみ。

### 設計意図

- `logger` は「ログは基本 guest にも読める」という世界観を利用し、追記された1行を大量のノイズ（既存ログ）の中から見つけさせることで間接性を上げる（スキンC「間接性が上がる」に対応）。
- `shredder` はゴールに直結する唯一の入力を破壊しうる、初めての「本当に危険に見える」選択肢。`inspect` の警告文を読んで自重するか、試してステージをやり直す羽目になるかを、プレイヤー自身の判断に委ねる。詰み防止は `back` による巻き戻しではなく、「いつでもステージ選択に戻ってやり直せる」という導線に一本化している（Overview.md「詰み防止」）。

### ゴール・クリア演出

- 鍵番号: `8264`
- 気づき: 「物騒な名前のツールほど、run する前に inspect で確かめる」
...
```json
{
  ...
  "tools": {
    ...
    "shredder": {
      ...
      "path_restriction": null,
      "destructive": true,
      "recoverable_via": null
    }
  },
  "notice": "物騒な名前のツールほど、run する前に inspect で確かめる"
}
```
...
- [ ] `inspect` `run` の初出ヒントは 1-2、`status` の初出ヒントは 2-2 に一度だけ出す（Screens.md 5章のNEWバッジ仕様に準拠）
- [ ] `back` コマンドは存在しない（v1.2で廃止）。1-4 の `shredder` を実行してしまった場合、そのセッション内での復旧手段はなく、「← ステージ選択に戻る」からのやり直しのみが復旧手段であることを実機で確認する
